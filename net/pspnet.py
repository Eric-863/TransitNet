...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone="resnet50", pretrained=True, aux_branch=True):
        ...
        # self.ela = EfficientLocalizationAttention(96, 7)
        # self.msaa = MSAA(out_channel, 320)
        #--------------------------------------------------------------#
        #	PSP模块，分区域进行池化
...
if __name__ == "__main__":
    ...
    pretrained = True
    model = PSPNet(num_classes=2, backbone=backbone, downsample_factor=downsample_factor, pretrained=pretrained).to(device)
