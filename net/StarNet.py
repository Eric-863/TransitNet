...
class StarNet(nn.Module):
    def __init__(self, base_dim=32, depths=[3, 3, 12, 5], mlp_ratio=4, drop_path_rate=0.0, num_classes=1000, **kwargs): #name="",
        super().__init__()
    ...
    def forward(self, x):   #修改
        x = self.stem(x)
        # savename = os.path.join(self.savepath, f"f{self.image_counter}.png")
        # draw_features(4, 8, x.cpu().detach().numpy(), savename)
        # print(savename)
        # self.image_counter += 1
        # i=0
